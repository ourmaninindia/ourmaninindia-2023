...
This web site is based on Hugo 0.110 with theme Ananke. All Ananka layouts etc were copied and adapted so in principle the theme is no longer required.
...
- Responsive
- Accessible
- Contact and subscribe form using Formspree
...
Take a look inside the config.toml for all the configurations.
...
For any page or post you can add a featured image by including the local path in front matter e.g.

`featured_image: '/images/gohugo-default-sample-hero-image.jpg'`

#### Featured image as Page Resources
We use Page Resources meaning all images are stored within the same content fonder. If no `featured_image` is set, the theme will look for a Page Resource of type `image` whose filepath incudes either `cover` or `feature` 
...
Custom CSS only works with Hugo Extended (installed here). 

In order to complement the default CSS, we have added a custom scss file to the project in the `assets/ananke/css` directory.
The file using the `custom_css` key in the site's parameter is registered in the config.toml file.  

The css file(s) will be added in their registered order to final `main.css` file.
...
You can also use predefined layout, like `:date_full`, and it will output localized dates or times. 
